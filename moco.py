...
parser.add_argument('--config_exp',default='./configs/pretext/moco_imagenet50.yml',
...
def main():
    ...
    print(model)
    model = torch.nn.DataParallel(model,[3])
    model = model.cuda(3)
    ...
    memory_bank_train = MemoryBank(len(train_dataset), 2048, p['num_classes'], p['temperature'],feature_dim=2048)
    memory_bank_train.cuda()
    memory_bank_val = MemoryBank(len(val_dataset), 2048, p['num_classes'], p['temperature'],feature_dim=2048)
