...
def klnihe(model,label_set,data,all_set,device):
    model.eval()
    import random
    ...
    subset_loader = DataLoader(data, batch_size=128,
                               sampler=SubsetSequentialSampler(all_set[:1000]),
                               pin_memory=True, drop_last=False)
    # data_loader = DataLoader(data, batch_size=128,
    #                            sampler=SubsetSequentialSampler(all_set[:1000]),
    #                            pin_memory=True, drop_last=False)

    sub = torch.tensor([]).to(device)
    all = torch.tensor([]).to(device)
    label = torch.LongTensor([]).to(device)
    ...
    for i in subset_loader:
        with torch.no_grad():
            ...
            output_batch,_ = model(input)
            # output_batch = torch.mean(output_batch,dim=1)
            # output_batch = softmax(output_batch)
            sub = torch.cat((sub, output_batch), 0)


            # output_augmented_batch,_ = model(input_augmented)
            # mean_in = torch.stack((output_batch,output_augmented_batch),dim =0)
            # mean_in = torch.mean(mean_in,dim =0)
            # print(mean_in.shape)
            # output_augmented_batch = torch.mean(output_augmented_batch, dim=1)
            # output_augmented_batch = softmax(output_augmented_batch)
            # sub = torch.cat((sub, mean_in), 0)
    # label = torch.squeeze(label)
    # print(label.shape[0])
    label = label.detach().cpu().numpy()
    targets = one_hot(label,10)
    ...
    #         input = input.to(device)
    #
    #         output_b,_ = model(input)
    #         # output_b = torch.mean(output_b,dim=1)
    #         # output_batch = softmax(output_batch)
    #         all = torch.cat((all, output_b), 0)
    #         # print(output_b.shape)

    # import  ipdb
    # ipdb.set_trace()
    print(colored(f'all:{all.size()},label:{sub.size()}','blue'))
    # p = torch.mean(sub,dim=0)
    q = torch.mean(all,dim=0)
    p_v = softmax(sub)
    print(p_v[0])
    # q_v = softmax(q)
    # log_mean = ((p_v + q_v )/2).log()

    # k_value = (kl(log_mean,q_v)+kl(log_mean,p_v))/2
    # k_value = torch.pow((p_v-targets),2)
    # # print(k_value)
    # k_value = torch.pow(k_value,0.5)/2
    # print(k_value.sum(1))
    k_value= kl(p_v.log(),targets)


    # return k_value.sum().item()/targets.shape[0]
    return k_value.item()
...
def activate_train(model,epoch,train_dataset,val_dataset,device):
    ...
    if is_c_loss == True:
        ...
        sched_backbone = lr_scheduler.MultiStepLR(optimizer, milestones=[100, 200])#[120,180]
        # optimizer = torch.optim.Adam(params=model.parameters(), lr=0.001, weight_decay=0.0001)
        # sched_backbone = lr_scheduler.MultiStepLR(optimizer, milestones=[200])  # cifar10[5,20]
        # c_loss = SimCLRLoss(0.1)
    else:
        optimizer = torch.optim.Adam(params=model.parameters(), lr=0.0001, weight_decay=0.0001)
        sched_backbone = lr_scheduler.MultiStepLR(optimizer, milestones=[20])#cifar10[5,20]
...
def main():
    ...
    #model
    p = create_config('./configs/env.yml', './configs/selflabel/selflabel_svhn.yml', 1, 10)
    print(colored(p, 'red'))
    device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")
    ...
    # all_dataset = get_train_dataset(p, val_transforms,
    ...
    # all_dataloader = torch.utils.data.DataLoader(train_dataset_, num_workers=8,
    #                                              batch_size=512, pin_memory=False, collate_fn=collate_custom,
    ...
    val_dataset_ = get_val_dataset(p, test_transforms)
    ...
    for i in range(epoch):
        ...
        kl = klnihe(model,label_set,train_dataset_,all_set,device)

        print(colored(f"active train lun ci : {i+1} acc: {acc} kl:{kl}", 'yellow'))
    ...
    # np.save('./results/cifar-10/kl(4).npy', pre_kl)
    np.save('./results/svhn/svhn(u8).npy', pre_acc)
