...
# Custom
from Constrat.config import *
from Constrat.models.query_models import VAE, Discriminator, GCN
from Constrat.data.sampler import SubsetSequentialSampler
from Constrat.kcenterGreedy import kCenterGreedy

from Constrat.alfa_mxi import ALFAMix
...
def get_uncertainty(models,loss_module, unlabeled_loader):
	models.eval()
	loss_module.eval()
	with torch.cuda.device(CUDA_VISIBLE_DEVICES):
...
def get_kcg(model, labeled_data_size, unlabeled_loader):
	model.eval()
	with torch.cuda.device(CUDA_VISIBLE_DEVICES):
		features = torch.tensor([]).cuda()
...
def query_samples(model, loss_module,method, data_train,data_unlabeled, subset, labeled_set, cycle, args):
	if method == 'Random':
	...
	if method == 'lloss':
		...
		uncertainty = get_uncertainty(model, loss_module,unlabeled_loader)
	...
	if method =='alfa_mix':
		net = model
		alfa = ALFAMix(data_unlabeled, data_train, net, args)
		querySet,remainSet = alfa.query(labeled_set,subset,1000)

		return querySet,remainSet
