...
from Constrat.strategy import Strategy, DatasetSplit


class ALFAMix(Strategy):
	def __init__(self, dataset_query, dataset_train, net, args):  # dataset_query is 没有变换的数据，data_train加了水平翻转
	...
	def query(self, label_idxs, unlabel_idxs, n_query=100):
		...
		if self.alpha_closed_form_approx:
			...
			out = self.net.cluster_head[0](var_emb)
		...
		remainSet = list(set(unlabel_idxs) - set(selected_idxs))
		querySet = list(selected_idxs)
	...
	def find_candidate_set(self, lb_embedding, ulb_embedding, pred_1, ulb_probs, alpha_cap, Y, grads):
		...
		for i in range(self.args.num_classes):
			...
			if self.alpha_closed_form_approx:
				...
				out = self.net.cluster_head[0](embedding_mix)
				out = out.detach().cpu()
				alpha = alpha.cpu()
				...
				if self.alpha_opt:
					...
					out = self.net.cluster_head[0](embedding_mix.to(self.args.device))
					out = out.detach().cpu()
	...
	def learn_alpha(self, org_embed, labels, anchor_embed, alpha, alpha_cap, log_prefix=''):
		...
		for i in range(self.alpha_learning_iters):
			...
			for b in range(math.ceil(float(alpha.size(0)) / self.alpha_learn_batch_size)):
				self.net.cluster_head[0].zero_grad()
