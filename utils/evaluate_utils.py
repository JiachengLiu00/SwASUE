...
def contrastive_evaluate(val_loader, model, memory_bank):
    ...
    for batch in val_loader:
        ...
        target = batch['target'].cuda(3,non_blocking=True)
...
def get_predictions(p, dataloader, model, return_features=False):
    ...
    if return_features:
        ...
        features = torch.zeros((len(dataloader.sampler), ft_dim)).cuda(3)
    ...
    for batch in dataloader:
        images = batch[key_].cuda(3,non_blocking=False)
        bs = images.shape[0]
...
def hungarian_evaluate(subhead_index, all_predictions, class_names=None, 
    ...
    targets = head['targets'].cuda(3)
    predictions = head['predictions'].cuda(3)
    probs = head['probabilities'].cuda(3)
    num_classes = torch.unique(targets).numel()
    num_elems = targets.size(0)

    match = _hungarian_match(predictions, targets, preds_k=num_classes, targets_k=num_classes)
    reordered_preds = torch.zeros(num_elems, dtype=predictions.dtype).cuda(3)
