...
def simclr_train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        input_ = input_.cuda(3,non_blocking=True)
        targets = batch['target'].cuda(3,non_blocking=True)
...
def scan_train(train_loader, model, criterion, optimizer, epoch, update_cluster_head_only=False):
    ...
    for i, batch in enumerate(train_loader):
        # Forward pass
        anchors = batch['anchor'].cuda(3,non_blocking=False)
        neighbors = batch['neighbor'].cuda(3,non_blocking=False)
...
def selflabel_train(train_loader, model, criterion, optimizer, epoch, ema=None):
    ...
    progress = ProgressMeter(len(train_loader), [losses],
                                prefix="Epoch: [{}]".format(epoch))
    model.train()
    print('model,device',colored(next(model.parameters()).device, 'red'))

    for i, batch in enumerate(train_loader):
        images = batch['image'].cuda(3,non_blocking=True)
        # print(colored(images.device,'red'))

        images_augmented = batch['image_augmented'].cuda(3,non_blocking=True)

        with torch.no_grad(): 
            output = model(images)[0]
        output_augmented = model(images_augmented)[0]
        # print(colored(output.device,'red'))
        # print(colored(output_augmented.device, 'red'))
