...
class ConfidenceBasedCE(nn.Module):
    ...
    def forward(self, anchors_weak, anchors_strong):
        ...
        max_prob, target = torch.max(weak_anchors_prob, dim = 1) #返回 最大值 和 最大值索引
...
class SimCLRLoss(nn.Module):
    ...
    def forward(self, features):
        ...
        assert(n == 2)
        mask = torch.eye(b, dtype=torch.float32).cuda(3)
        ...
        logits_mask = torch.scatter(torch.ones_like(mask), 1, torch.arange(b).view(-1, 1).cuda(3), 0)
